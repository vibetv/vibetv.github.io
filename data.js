    ...
    playerInstance.on('error', function() {
        ...
        if (channel.iframeLink) {
            ...
            playerContainer.innerHTML = `<div class="iframe-container">
                        <iframe src="${channel.iframeLink}" frameborder="0"allow-scripts allow-same-origin" allowfullscreen="true"></iframe>
                    </div>`;
        }
    });
...
fetch('https://vibetv.github.io/channel.json')
    .then(response => response.json())
    .then(data => {
        const channelsContainer = document.getElementById('channels');
        data.forEach(channel => {
            const channelDiv = document.createElement('div');
            channelDiv.className = `list-group-item channel ${channel.category}`;
